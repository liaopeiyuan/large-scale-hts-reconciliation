...
Eigen::MatrixXi construct_S(const Eigen::MatrixXi S_compact, int num_base,
  ...
  for (int i = 0; i < num_base; i++) {
    ...
    if (co >= num_base) {
      throw std::invalid_argument(
          "Make sure that the frist num_base rows of "
    ...
    for (int j = 1; j < num_levels; j++) {
      ...
      if (ro == -1) {
        if (i < num_base) {
          throw std::invalid_argument(
              "Make sure that the frist num_base rows of S_compact contain "
        ...
        if (co >= num_base) {
          throw std::invalid_argument(
              "Make sure that the all leaf-level nodes have index < num_base.");
        }
        S(ro, co) = 1;
      }
    }
...
Eigen::MatrixXf construct_G_top_down(const Eigen::MatrixXi S_compact,
                                     const Eigen::MatrixXf P, int num_base,
                                     int num_total, int num_levels) {
...
Eigen::MatrixXf construct_G_bottom_up(const Eigen::MatrixXi S_compact,
                                      int num_base, int num_total,
                                      int num_levels) {
  Eigen::MatrixXf G = Eigen::MatrixXf::Zero(num_base, num_total);
...
Eigen::MatrixXf dp_reconcile_optimized(const std::string method,
                                       ...
                                       const Eigen::MatrixXf P,
                                       const Eigen::MatrixXf yhat, int level,
...
Eigen::MatrixXf construct_dp_reconciliation_matrix(
    const std::string method, const Eigen::MatrixXi S_compact,
    const Eigen::MatrixXf P, int level, float w, int num_base, int num_total,
    int num_levels, int slice_start, int slice_length) {
  Eigen::MatrixXi S = construct_S(S_compact, num_base, num_total, num_levels);
  ...
  if (method == "bottom_up") {
    ...
    G = construct_G_middle_out(S_compact, P, level, num_base, num_total,
                               num_levels);
  } else if (method == "OLS") {
    G = construct_G_OLS(S);
  } else if (method == "WLS") {
...
Eigen::MatrixXf reconcile_matrix(const std::string method,
                                 const Eigen::MatrixXi S_compact,
                                 const Eigen::MatrixXf P,
                                 const Eigen::MatrixXf yhat, int level, float w,
...
Eigen::MatrixXf reconcile(const std::string method,
                          ...
                          int level, float w, int num_base, int num_total,
                          int num_levels) {
  Eigen::MatrixXi S = construct_S(S_compact, num_base, num_total, num_levels);

  // std::stringstream ss;
  // ss << S.rows() << " " << S.cols() << " " << S(Eigen::seqN(0, 10),
  // Eigen::seqN(0, 10)); printf("S: %s\n", ss.str().c_str());
 ...
 public:
  ...
  Eigen::MatrixXf reconcile_dp_optimized(const std::string method,
    ...
    if (method == "bottom_up") {
      ...
      Eigen::MatrixXf yhat_total = Eigen::MatrixXf::Zero(
          std::accumulate(rows.begin(), rows.end(), 0), cols[0]);
...
*/
      ...
      Eigen::MatrixXi S =
          construct_S(S_compact, num_base, num_total, num_levels)
 ...
 */
  ...
  Eigen::MatrixXf reconcile_gather(const std::string method,
    ...
    if (world_rank == 0) {
      ...
      yhat_total = Eigen::MatrixXf::Zero(
          std::accumulate(rows.begin(), rows.end(), 0), cols[0]);
    ...
    if (world_rank == 0) {
      ...
      Eigen::MatrixXf y_reconciled =
          reconcile(method, S_compact, P, yhat_total, level, w, num_base,
                    num_total, num_levels);
