...
def main():
    ...
    start = timer()
    rec2 = distrib.reconcile_dp_optimized("top_down", S_compact, top_down_p, y_hat, -1, 0.0, 5650, 6218, 4)
    end = timer()
    elapsed = round(end - start, 4)
    if (rank == size - 1): 
        print("Top down (dp optimized): ", str(elapsed), " ", lhts.smape(rec2, gt))
    ...
    start = timer()
    rec2 = distrib.reconcile_dp_optimized("middle_out", S_compact, level_2_p, y_hat, 2, 0.0, 5650, 6218, 4)
    end = timer()
    elapsed = round(end - start, 4)
    if (rank == size - 1): 
        print("Middle out (dp optimized): ", str(elapsed), " ", lhts.smape(rec2, gt))
