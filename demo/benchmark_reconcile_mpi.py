...
def main():
    ...
    start = timer()
    rec = distrib.reconcile_dp_matrix(METHOD, S_compact, 5650, 6218, 4, yhat, level_2_p, 2, 0.0)
    end = timer()
    ...
    start = timer()
    rec2 = distrib.reconcile_dp_optimized(METHOD, S_compact, 5650, 6218, 4, yhat, level_2_p, 2, 0.0)
    end = timer()
    elapsed = round(end - start, 4)
    if (rank == size - 1): 
    ...
    start = timer()
    rec3 = distrib.reconcile_gather(METHOD, S_compact, 5650, 6218, 4, yhat, level_2_p, 2, 0.0)
    end = timer()
    elapsed = round(end - start, 4)
    if (rank == size - 1):
