...
def main():
    ...
    if METHOD == "middle_out":
        P = np.load(open(data_dir + DATA_ROOT + "/top_down_tensor.npy", "rb"))[
            :, 0
        ].reshape(-1, 1)
    else:
        P = np.load(open(data_dir + DATA_ROOT + "/level_2_tensor.npy", "rb"))[
            :, 0
        ].reshape(-1, 1)
    ...
    df_slice = df.iloc[
        len_per_rank * rank : min(total_len, len_per_rank * (rank + 1)), :
    ...
    for i, row in it:
        data = pd.DataFrame(
            {
                "ds": (row.index)[1:-4][-2 * TIME_HORIZON : -TIME_HORIZON],
                "y": (row.values)[1:-4][-2 * TIME_HORIZON : -TIME_HORIZON],
            }
        )
    ...
    start = timer()
    rec = distrib.reconcile_dp_optimized(
        METHOD, S_compact, 5650, 6218, 4, yhat, P, 2, 1.5
    )
    end = timer()
    elapsed = round(end - start, 4)
    if rank == 0:
        print(
            "Reconciliation with " + METHOD + " done: ",
            str(elapsed),
            " ",
            lhts.smape(yhat, gt),
            " -> ",
